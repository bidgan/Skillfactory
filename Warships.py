...
# здесь просто задаем цвета, при желании цвета можно легко поменять
class Color:
    yellow2 = '\033[1;35m'
    reset = '\033[0m'
    blue = '\033[0;34m'
...
# поле игры, радар и кф клеток
# Радар нужен чтобы "отмечать" места промахов и попаданий, по сути вторая карта для удобства игрока
...
# В классе Game мы опишем цифры используемые для отображения координат.
class Game(object):
    letters = ("1", "2", "3", "4", "5", "6")
    # все возможные размеры кораблей и их количество
        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    print('{}. Подготовка игрового поля...'.format(player.name))

                player.message.clear()

                x, y, r = player.get_input('ship_setup')
                # если пользователь ввёл что то не то, функция возвратит нули, пишем continue
    ...
    def draw(self):
        if not self.current_player.is_ai:
            ...
            #вес клеток, вероятность попадания
